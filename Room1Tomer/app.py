...
def main():
    ...
    if is_training:
        ...
        # Room 5 has a custom sidebar / preview / training flow in the incoming branch
        if room == 5:
    ...
    if room in (1, 2):
        ...
        render_grid_editor()
        if is_training:
            # Preserve the main branch locked-training flow (was Room 1 specific)
            train_room1(params)
            st.session_state["is_training"] = False
            st.rerun()
        elif train_clicked:
            # Incoming branch generalized training for Room 1 vs Room 2
