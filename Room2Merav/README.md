...
it. An access card, tucked away from the direct route, unlocks the exit
...
- the same `(row, col)` cell means something different depending on
  whether the access card was already collected (`G` only ends the
...
| Reaching the access card (`K`), first time | `-1 + 10` (step cost + one-time subgoal bonus) |
| Reaching the door (`G`) **while holding the card** | `max(100 - steps_taken, 20)`, episode ends |
| Reaching `G` **without** the card | `-1` — door is locked, just a normal floor cell, episode continues |
...
higher the reward" requirement. The card's one-time `+10` is a subgoal
reward-shaping bonus: without it, the only feedback distinguishing "have
card" from "no card" is the (temporally distant) exit payoff, which gives
...
`key_bonus` — for simplicity; they refer to the abyss and the access card
respectively.)
...
itself. The access card sits on the near (start) side of the bridge, so
...
`S` start · `K` access card · `G` door/exit (locked until card collected) ·
...
`alpha=0.3, gamma=0.95, epsilon_decay=0.995`, averaged over several seeds):

- **Final learned (greedy) policies were nearly identical.** Across 4
  seeds, SARSA and Q-Learning converged to the same route in 3/4 cases
  (~96% success, ~41 steps, ~0.16 average lateral moves along the risky
  part of row 6) — confirming that when the danger is environmental
  rather than exploration-induced, there's no strong reason for one
  algorithm's final policy to be systematically safer than the other's.
- **Training-time return was consistently different, though.** Averaged
  over the last 500 training episodes (with `epsilon_min=0.05` still
  active), SARSA reliably scored higher (~15.5 across all 4 seeds) than
  Q-Learning (7.4-13.3, more seed-dependent):
...
These are `TrainRoom2Config`'s defaults: ~98.75% greedy success at ~42
steps, converging by ~episode 600 — see `sweep_results.csv` for the full
grid. Full run (3000 episodes, these defaults):

![Learning curves](learning_curve.png)

- Success rate: 0% (first 50 episodes) → ~90-96% (last 50 episodes,
  training-time — still with `epsilon_min=0.05` exploration noise)
- Average steps on success: ~41-46, consistent with the ~40-step
